...
teacher_ckpt_path=${TEACHER_CKPT_PATH:-/kaggle/working/KDFS-Filter-Pruning-Fixed-KD-Loss/teacher_dir/teacher_model_best.pth}
device=${DEVICE:-0,1}
...
if [ "$PHASE" = "train" ]; then
    echo "torchrun --nproc_per_node=$nproc_per_node --master_port=$master_port /kaggle/working/KDFS-Filter-Pruning-Fixed-KD-Loss/main.py \
        --phase train \
        --arch $arch \
        --device cuda \
...
if [ "$PHASE" = "train" ]; then
    torchrun --nproc_per_node=$nproc_per_node --master_port=$master_port /kaggle/working/KDFS-Filter-Pruning-Fixed-KD-Loss/main.py \
        --phase train \
        --arch "$arch" \
        --device cuda \
        --result_dir "$result_dir" \
